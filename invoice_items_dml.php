...
function invoice_items_insert(){
	...
	sql('insert into `invoice_items` set       `item`=' . (($data['item'] !== '' && $data['item'] !== NULL) ? "'{$data['item']}'" : 'NULL') . ', `unit_price`=' . (($data['unit_price'] !== '' && $data['unit_price'] !== NULL) ? "'{$data['unit_price']}'" : 'NULL') . ', `qty`=' . (($data['qty'] !== '' && $data['qty'] !== NULL) ? "'{$data['qty']}'" : 'NULL') . ', `price`=' . (($data['price'] !== '' && $data['price'] !== NULL) ? "'{$data['price']}'" : 'NULL'), $o);
	if($o['error']!=''){
		echo $o['error'];
		echo "<a href=\"invoice_items_view.php?addNew_x=1\">{$Translation['< back']}</a>";
		exit;
	}
	...
	sql("insert ignore into membership_userrecords set tableName='invoice_items', pkValue='" . makeSafe($recID, false) . "', memberID='" . makeSafe(getLoggedMemberID(), false) . "', dateAdded='" . time() . "', dateUpdated='" . time() . "', groupID='" . getLoggedGroupID() . "'", $eo);
...
function invoice_items_update($selected_id){
	...
	$o=array('silentErrors' => true);
	sql('update `invoice_items` set       `item`=' . (($data['item'] !== '' && $data['item'] !== NULL) ? "'{$data['item']}'" : 'NULL') . ', `unit_price`=' . (($data['unit_price'] !== '' && $data['unit_price'] !== NULL) ? "'{$data['unit_price']}'" : 'NULL') . ', `qty`=' . (($data['qty'] !== '' && $data['qty'] !== NULL) ? "'{$data['qty']}'" : 'NULL') . ', `price`=' . (($data['price'] !== '' && $data['price'] !== NULL) ? "'{$data['price']}'" : 'NULL') . " where `id`='".makeSafe($selected_id)."'", $o);
	if($o['error']!=''){
		echo $o['error'];
...
function invoice_items_form($selected_id = '', $AllowUpdate = 1, $AllowInsert = 1, $AllowDelete = 1, $ShowCancel = 0, $TemplateDV = '', $TemplateDVP = ''){
	...
	if($selected_id){
		...
		$templateCode=str_replace('<%%VALUE(qty)%%>', html_attr($row['qty']), $templateCode);
	...
	$templateCode .= loadView('invoice_items-ajax-cache', $cache_data);
